...
public class Helper {
    ...
    public static Double getDistance(DataPoint a, DataPoint b, List<SimilarityMatrix> similarityMatrices) {
        ...
        for (Feature feat: a.getFeatures()) {
            if (feat.getFeatureType() == FeatureType.CATEGROICAL) {
                for (SimilarityMatrix sm: similarityMatrices) {
                    if (sm.getFeatLabel().equals(feat.getLabel())) {
                        totalDistance += sm.getDistance(feat.getCategoricalPayload(), b.getFeatureByLabel(feat.getLabel()).getCategoricalPayload());
                    }
                }
            } else if (feat.getFeatureType() == FeatureType.CONTINUOUS) {
    ...
    public static String vote(DataPoint[] data){
        ...
        return maxKey; //Return key with most votes
    }
